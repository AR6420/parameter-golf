...
try:
    ...
    # Fallback for non-Hopper GPUs (local dev on RTX 5070 Ti etc.)
    def flash_attn_3_func(q, k, v, causal=False, **kwargs):
        q = q.transpose(1, 2).contiguous()  # [B,T,H,D] -> [B,H,T,D]
        ...
        return out.transpose(1, 2)  # [B,H,T,D] -> [B,T,H,D]
...
if _USE_TRITON_MLP:
    ...
    def _fused_mlp_up_proj_kernel(
        ...
        BLOCK_M: tl.constexpr = 128, BLOCK_N: tl.constexpr = 128, BLOCK_K: tl.constexpr = 32, GROUP_SIZE_M: tl.constexpr = 8,
    ):
        pid = tl.program_id(0)
        ...
        acc = tl.where(acc > 0, acc, acc * 0.5)  # LeakyReLU(0.5)
        acc = acc * acc                           # square
    ...
    _DOWN_PROJ_CONFIGS = [
        ...
        triton.Config({'BLOCK_M': 64,  'BLOCK_N': 128, 'BLOCK_K': 32, 'GROUP_SIZE_M': 8}, num_stages=4, num_warps=4),
        triton.Config({'BLOCK_M': 64,  'BLOCK_N': 64,  'BLOCK_K': 32, 'GROUP_SIZE_M': 8}, num_stages=5, num_warps=2),
        triton.Config({'BLOCK_M': 128, 'BLOCK_N': 128, 'BLOCK_K': 64, 'GROUP_SIZE_M': 8}, num_stages=3, num_warps=4),
        ...
        triton.Config({'BLOCK_M': 128, 'BLOCK_N': 64,  'BLOCK_K': 128, 'GROUP_SIZE_M': 8}, num_stages=3, num_warps=4),
    ...
    def _fused_mlp_down_proj_kernel(
        a_ptr, b_ptr, c_ptr, scale_ptr, res_ptr, M, N, K,
        stride_am, stride_ak, stride_bk, stride_bn, stride_cm, stride_cn, stride_rm, stride_rn,
        BLOCK_M: tl.constexpr = 128, BLOCK_N: tl.constexpr = 64, BLOCK_K: tl.constexpr = 64, GROUP_SIZE_M: tl.constexpr = 8,
    ):
        pid = tl.program_id(0)
        num_pid_m = tl.cdiv(M, BLOCK_M)
        num_pid_n = tl.cdiv(N, BLOCK_N)
...
def _triton_mlp_up_proj_fwd(x: Tensor, weight: Tensor) -> Tensor:
    """Raw Triton forward: out = LeakyReLU(x @ weight.T, 0.5)^2"""
    orig_shape = x.shape
    orig_dtype = x.dtype
...
def _triton_mlp_down_proj_fwd(hidden: Tensor, weight: Tensor, mlp_scale: Tensor, residual: Tensor) -> Tensor:
    """Raw Triton forward: out = residual + mlp_scale * (hidden @ weight.T)"""
    orig_shape = hidden.shape
    hidden_2d = hidden.reshape(-1, hidden.shape[-1]).to(torch.bfloat16)
    weight = weight.to(torch.bfloat16)
    mlp_scale = mlp_scale.to(torch.bfloat16)
    res_2d = residual.reshape(-1, residual.shape[-1]).to(torch.bfloat16)
    M, K = hidden_2d.shape
    N = weight.shape[0]
    ...
    _fused_mlp_down_proj_kernel[grid](
        hidden_2d, weight, out, mlp_scale, res_2d, M, N, K,
...
class _FusedMLPUpProj(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, weight):
        ctx.save_for_backward(x, weight)
        return _triton_mlp_up_proj_fwd(x, weight)
    @staticmethod
    def backward(ctx, grad_output):
        x, weight = ctx.saved_tensors
        # Recompute intermediate for backward (standard PyTorch ops)
        pre_act = F.linear(x, weight)
        activated = F.leaky_relu(pre_act, negative_slope=0.5)
...
class _FusedMLPDownProj(torch.autograd.Function):
    @staticmethod
    def forward(ctx, hidden, weight, mlp_scale, residual):
        ctx.save_for_backward(hidden, weight, mlp_scale)
        return _triton_mlp_down_proj_fwd(hidden, weight, mlp_scale, residual)
    @staticmethod
    def backward(ctx, grad_output):
...
def fused_mlp_up_proj(x: Tensor, weight: Tensor) -> Tensor:
    ...
    return _FusedMLPUpProj.apply(x, weight)

def fused_mlp_down_proj(hidden: Tensor, weight: Tensor, mlp_scale: Tensor, residual: Tensor) -> Tensor:
    """out = residual + mlp_scale * (hidden @ weight.T) -- fused forward, PyTorch backward."""
    if not _USE_TRITON_MLP:
...
def eval_val(
    ...
    with torch.inference_mode():
        for _batch_idx, batch_seq_start in enumerate(range(seq_start, seq_end, local_batch_seqs)):
            if _val_max_batches > 0 and _batch_idx >= _val_max_batches:
                break
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor,
                mlp_scale: Tensor | None = None, residual: Tensor | None = None) -> Tensor:
        ...
        x = F.leaky_relu(F.linear(x, up_w_cast), negative_slope=0.5)
        return F.linear(x.square(), down_w_cast)
